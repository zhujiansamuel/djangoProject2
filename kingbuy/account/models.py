...
from kingbuy.core.models import ModelWithMetadata
...
class User(PermissionsMixin, ModelWithMetadata, AbstractBaseUser):
    ...
    addresses_2 = models.ManyToManyField(
        Address, blank=True, related_name="user_addresses_2"
    )
    age = models.CharField(max_length=6, blank=True, null=True)
    gender = models.CharField( max_length=256, blank=True, null=True )
    birth = models.DateField( default=timezone.now, null=True )
    ...
    class Meta:
        permissions = (
            ...
            ("manage_staff", pgettext_lazy("Permission description", "スタッフ管理")),
            (
                "impersonate_users",
...
class CustomerEvent(models.Model):
    ...
    order_manage = models.ForeignKey(OrderManage, on_delete=models.SET_NULL, null=True)
    barter_manage = models.ForeignKey(BarterManage, on_delete=models.SET_NULL, null=True)
    store_to_store_manage = models.ForeignKey(StoreToStoreManage, on_delete=models.SET_NULL, null=True)
    manual_inventory_manage = models.ForeignKey(ManualInventoryManage, on_delete=models.SET_NULL, null=True)
